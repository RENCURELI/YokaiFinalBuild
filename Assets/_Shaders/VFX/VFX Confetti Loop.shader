Shader "LOCAL/VFX/Confetti Loop"
{
    Properties
    {
	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 ParticleTrajectory(float3 center, float3 lifetime, float seed) {
				...
				return origin + normalize(toCenter) * lerp(_Spread2, _Spread, pow(lifetime,0.8)) * length(toCenter) * abs(sin(seed*20)) + spin * abs(cos(seed*30));
			}
