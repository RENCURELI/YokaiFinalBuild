Shader "LOCAL/VFX/Candle Sparkle Burst"
{
    Properties
    {
	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 ParticleTrajectory(float3 center, float3 lifetime, float seed) {
				...
				return origin + float3(0,1,0) * _Amplitude * lifetime + normalize(toCenter) * lerp(_Spread2, _Spread, lifetime) * length(toCenter) * abs(sin(seed*20)) + spin * abs(cos(seed*30));
			}

			float Lifetime(float seed) {
				return _Lifetime + abs(cos(seed*30)) * 0.1 * _Lifetime;
			}
