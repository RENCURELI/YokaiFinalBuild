	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 ParticleTrajectory(float3 center, float3 lifetime, float seed) {
				...
				return origin + toCenter - normalize(toCenter)*0.5 + normalize(toCenter) * lerp(_Spread2, _Spread, lifetime) + spin * abs(cos(seed*30));
			}
			...
			float4 FragmentProgram(GenericFragmentInput input) : SV_TARGET{
				...
				return saturate(color * _Emit * _Intensity);
			}
