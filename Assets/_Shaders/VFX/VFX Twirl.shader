	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 ParticleTrajectory(float3 center, float3 lifetime, float seed) {
				...
				float twirlAngle = center + seed * 37 + pow(lifetime, 5) * _Speed;
				float3 twirl = mul(Rotation(ObjectDirection(float3(0, 1, 0)), twirlAngle), float4(toCenter, 0)).xyz;
				return origin + normalize(twirl) * lerp(_Spread2, _Spread, lifetime) + spin * abs(cos(seed*30));
			}
